...
config = load_json_from_file('res_mods/configs/goldvisibility2.json')
        ...
        for (index, file_path) in enumerate(pixie_effect._files):
            ...
            if match is not None:
                ...
                if shell_type in gold_ammo_types and effect_must_show(player, players, shell_type, gun, attacker_id):
                    modified_file_name_mgr.add_value(pixie_effect._files, index, pixie_effect._files[index])
                    pixie_effect._files[index] = 'particles/Shells_Eff/' + effect_name + '_prem.xml'
